...
float pointLightShadow(float distanceFromCamera, float shadowFalloffDistance, samplerCube shadowMap, vec3 lightPos, mat4 lightMatrix, vec3 worldPosition) {
...
vec3 computePointLights (float distanceFromCamera, samplerCube shadowMap, mat4 pointLight, vec3 worldPosition, vec3 V, vec3 N, float roughness, float metallic, vec3 F0) {
    ...
    if (hasShadowMap) shadows = pointLightShadow(distanceFromCamera, pointLight[3][2], shadowMap, lightPosition, pointLight, worldPosition);
