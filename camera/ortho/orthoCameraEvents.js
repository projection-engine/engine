...
export default function orthoCameraEvents(camera, canvasID, onClick) {
    let target = document.getElementById(canvasID)
    let isFocused = false
    let startMouseDown
    const maxAngle = camera instanceof OrthographicCamera ? 1.57 : 1.4
        ...
        switch (event.type) {
            ...
            case 'mousemove': {
                ...
                if (isFocused) {
                    ...
                    if ((event.movementX < 0 && camera instanceof OrthographicCamera) || (event.movementX > 0 && camera instanceof SphericalCamera))
                        camera.yaw += (conf.sensitivity.yaw ? conf.sensitivity.yaw : .005) * Math.abs(event.movementX)
                    else if ((event.movementX > 0 && camera instanceof OrthographicCamera) || (event.movementX < 0 && camera instanceof SphericalCamera))
                        camera.yaw -= (conf.sensitivity.yaw ? conf.sensitivity.yaw : .005) * Math.abs(event.movementX)
