...
export default function packageDirectionalLights(keepOld) {
    let directionalLightsData = LightsAPI.directionalLightsBuffer,
        ...
        offset = 0,
        directionalLights = LightsAPI.directionalLights.array,
        size = 0
    ...
    for (let i = 0; i < directionalLights.length; i++) {
        ...
        const position = vec3.add([], current.absoluteTranslation, component._center)
        ...
        directionalLightsData[offset + 14] = component.hasSSS ? 1 : 0

        if (component.shadowMap) {
            mat4.lookAt(component.lightView, component.__entity.absoluteTranslation, component.center, [0, 1, 0])
    ...
    LightsAPI.updateDirectionalLightsBuffer()
}