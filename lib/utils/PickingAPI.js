...
export default class PickingAPI {
    static readBlock(start, end) {
        ...
        gpu.bindFramebuffer(gpu.FRAMEBUFFER, VisibilityBuffer.buffer.FBO)
        gpu.readBuffer(gpu.COLOR_ATTACHMENT0 + 2)
    ...
    static readUV(x, y, imageWidth, imageHeight) {
        ...
        const uv = PickingAPI.readPixels(VisibilityBuffer.buffer.FBO, 3, coords)
        const texCoords = {
    ...
    static readEntityID(x, y) {
        const w = window.gpu.canvas.width, h = window.gpu.canvas.height
        const coords = ConversionAPI.toQuadCoord({x, y}, {w, h})
        const picked = PickingAPI.readPixels(VisibilityBuffer.buffer.FBO, 2, coords)
