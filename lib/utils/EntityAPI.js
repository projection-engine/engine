...
export default class EntityAPI {
    ...
    static registerEntityComponents(entity, previouslyRemoved) {
        ...
        console.trace("HI IM HERE")
        const data = Engine.data
        Engine.queryMap.set(entity.queryKey, entity)
        let placementMap = EntityAPI.dataEntity.get(entity.id) || {}
        ...
        if (entity.components.get(COMPONENTS.DIRECTIONAL_LIGHT) && !placementMap.directionalLights) {
            data.directionalLights.push(entity)
            placementMap.directionalLights = true
        }

        if (entity.components.get(COMPONENTS.SPOTLIGHT) && !placementMap.spotLights) {
            data.spotLights.push(entity)
            placementMap.spotLights = true
        }

        if (entity.components.get(COMPONENTS.MESH) && !placementMap.meshes) {
            data.meshes.push(entity)
            MaterialAPI.updateMap(entity.components.get(COMPONENTS.MESH))
            placementMap.meshes = true
        }

        if (entity.components.get(COMPONENTS.CAMERA) && !placementMap.cameras) {
            data.cameras.push(entity)
            placementMap.cameras = true
        }

        if (entity.components.get(COMPONENTS.SKYLIGHT) && !placementMap.skylights) {
            data.skylights.push(entity)
            placementMap.skylights = true
        }

        if (entity.components.get(COMPONENTS.SPRITE) && !placementMap.sprites) {
            data.sprites.push(entity)
            placementMap.sprites = true
        }

        if (entity.components.get(COMPONENTS.TERRAIN) != null && !placementMap.terrain) {
        ...
        const toFind = [COMPONENTS.POINT_LIGHT, COMPONENTS.DIRECTIONAL_LIGHT, COMPONENTS.SPOTLIGHT, COMPONENTS.MESH]
        EntityAPI.dataEntity.set(entity.id, placementMap)
        console.trace(previouslyRemoved, toFind)
        if (toFind.indexOf(previouslyRemoved) || !!toFind.find(v => entity.components.get(v) != null))
    ...
    static removeEntity(id) {
        ...
        const placementMap = EntityAPI.dataEntity.get(id)
        Object.keys(placementMap).forEach(k => Engine.data[k] = Engine.data[k].filter(e => e !== entity))
        ...
        if (entity.components.get(COMPONENTS.POINT_LIGHT) || entity.components.get(COMPONENTS.DIRECTIONAL_LIGHT) || entity.components.get(COMPONENTS.MESH))
            LightsAPI.packageLights(false, true)
        VisibilityBuffer.needsUpdate = true
        EntityAPI.dataEntity.delete(id)
    }
