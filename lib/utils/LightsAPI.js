...
export default class LightsAPI {
    ...
    static typeBufferA = ArrayBufferAPI.allocateVector(MAX_LIGHTS * 16, 0, false, false, true)
    static typeBufferB = ArrayBufferAPI.allocateVector(MAX_LIGHTS * 16, 0, false, false, true)
    static typeBufferC = ArrayBufferAPI.allocateVector(MAX_LIGHTS * 16, 0, false, false, true)
    ...
    static #package(keepOld) {
        ...
        for (let i = 0; i < toLoopSize; i++) {
            ...
            if (offset + 16 > MAX_LIGHTS && primaryBuffer === LightsAPI.primaryBufferC || !current.active || !current.changesApplied && !current.needsLightUpdate && keepOld)
                continue
            LightsAPI.updateBuffer(current, primaryBuffer, secondaryBuffer, typeBuffer, offset)
            offset += 16
            size[sizeIndex] += 1
        }
        console.log(primaryBuffer, size, typeBuffer)
